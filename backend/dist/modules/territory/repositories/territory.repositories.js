...
class TerritoryRepository {
     ...
     */
    async createMunicipalityWithHierarchy(dto) {
        ...
        try {
            ...
            throw new appErrors_1.BadRequestError('Erreur lors de la création de la hiérarchie territoriale');
        }
        finally {
            client.release();
        }
    ...
    async getRegions() {
        try {
            ...
            throw new appErrors_1.BadRequestError('Erreur lors de la récupération des régions');
        }
    }
    async getMunicipalitiesByRegion(regionId) {
        try {
            ...
            throw new appErrors_1.BadRequestError('Erreur lors de la récupération des municipalités');
        }
    }
    async getDistrictsByMunicipality(municipalityId) {
        try {
            ...
            throw new appErrors_1.BadRequestError('Erreur lors de la récupération des arrondissements');
        }
    }
    async getNeighborhoodsByDistrict(districtId) {
        try {
            ...
            throw new appErrors_1.BadRequestError('Erreur lors de la récupération des quartiers');
        }
    }
    /**
     ...
     */
    async getFullTerritoryHierarchy(organizationId) {
        try {
            if (organizationId) {
                const sql = `
                    SELECT 
            ...
            throw new appErrors_1.BadRequestError('Erreur lors de la récupération de la hiérarchie territoriale');
        }
    }
    async deleteMunicipality(id) {
        try {
            ...
            throw new appErrors_1.BadRequestError('Erreur lors de la suppression de la municipalité');
        }
    }
    /**
     * Récupère les informations territoriales et les rôles d'un utilisateur.
     */
    async getUserTerritoryInfo(userId) {
        try {
            const userSql = `
                SELECT 
                    u.id, 
            ...
            if (userResult.rows.length === 0) {
                throw new appErrors_1.BadRequestError("Utilisateur non trouvé");
            }
            return userResult.rows[0];
        }
        catch (error) {
            this.logger.error('Error fetching user territory info:', error);
            throw new appErrors_1.BadRequestError('Erreur lors de la récupération des informations utilisateur');
        }
    }
    /**
    ...
    async getUserBoundary(userId) {
        try {
            ...
            throw new appErrors_1.BadRequestError('Erreur lors du calcul des limites territoriales');
        }
    }
    /**
     * Trouve la municipalité contenant la coordonnée GPS (latitude, longitude) spécifiée.
     */
    async getMunicipalityByCoords(longitude, latitude) {
        try {
            ...
            throw new appErrors_1.BadRequestError('Erreur lors du géocodage inverse');
        }
    }
    /**
     ...
     */
    async getGeoJsonByLevel(level, options = {}) {
        ...
        try {
            if (redisConfig_1.default.isOpen) {
                const cached = await redisConfig_1.default.get(cacheKey);
                if (cached)
        ...
        try {
            ...
            throw new appErrors_1.BadRequestError(`Erreur lors de la récupération des géométries (${level})`);
        }
    }
    defaultTolerance(level) {
        switch (level) {
            case 'regions':
                return 0.01;
            case 'municipalities':
    ...
    async getHierarchyByCoords(longitude, latitude) {
        try {
            const sql = `
                SELECT 
            ...
            throw new appErrors_1.BadRequestError('Erreur lors du géocodage inverse complet');
        }
    }
    /**
     * GetMunicipalityById
    ...
    async getMunicipalityById(id) {
        try {
            const result = await this.db.query(`SELECT * FROM municipalities WHERE id = $1`, [id]);
            return result.rows[0] || null;
        }
        catch (error) {
            this.logger.error('Error fetching municipality by id:', error);
            throw new appErrors_1.BadRequestError('Erreur lors de la récupération de la municipalité');
        }
    }
    /**
     * UpdateMunicipality
    ...
    async updateMunicipality(id, dto) {
        try {
            ...
            throw new appErrors_1.BadRequestError('Erreur lors de la mise à jour de la municipalité');
        }
    }
    // ============================================================================
    // GIS UPSERT METHODS
    ...
    extractFeatureProps(feature) {
        ...
        const name = (props.name || props.nom || props.label || props.title || 'Inconnu');
        const code = (props.code || props.id || null);
        return { name, code };
    }
    async upsertRegions(features) {
        ...
        try {
            ...
            throw new appErrors_1.BadRequestError('Erreur lors de la mise à jour des régions via SIG');
        }
        finally {
            client.release();
        }
    }
    async upsertMunicipalities(features) {
        if (features.length === 0)
            return;
        const client = await this.db.getClient();
        try {
            ...
            for (let i = 0; i < features.length; i += BATCH_SIZE) {
                ...
                for (const feature of chunk) {
                    if (!feature.geometry)
                        continue;
                    const { name, code } = this.extractFeatureProps(feature);
                    const department = (feature.properties?.department || feature.properties?.departement || null);
                    const geomJson = JSON.stringify(feature.geometry);
            ...
            throw new appErrors_1.BadRequestError('Erreur lors de la mise à jour des communes via SIG');
        }
        finally {
            client.release();
        }
    }
    async upsertDistricts(features) {
        if (features.length === 0)
            return;
        const client = await this.db.getClient();
        try {
            ...
            throw new appErrors_1.BadRequestError('Erreur lors de la mise à jour des arrondissements via SIG');
        }
        finally {
            client.release();
        }
    }
    async upsertNeighborhoods(features) {
        if (features.length === 0)
            return;
        const client = await this.db.getClient();
        try {
            ...
            throw new appErrors_1.BadRequestError('Erreur lors de la mise à jour des quartiers via SIG');
        }
        finally {
            client.release();
        }
