...
class ReportsRepository {
    ...
    async insertExtension(client, reportId, dto) {
        const ext = dto.issueCategory;
        if (ext === reports_types_1.IssueCategory.DRAINAGE && dto.drainageDetails) {
            await client.query(`INSERT INTO report_details_drainage (report_id, blockage_level_pct, water_level_cm, flow_status)
                 VALUES ($1, $2, $3, $4)`, [reportId, dto.drainageDetails.blockageLevelPct, dto.drainageDetails.waterLevelCm, dto.drainageDetails.flowStatus]);
        }
        else if (ext === reports_types_1.IssueCategory.ROAD && dto.roadDetails) {
            await client.query(`INSERT INTO report_details_road (report_id, damage_surface_m2, pothole_depth_cm)
                 VALUES ($1, $2, $3)`, [reportId, dto.roadDetails.damageSurfaceM2, dto.roadDetails.potholeDepthCm]);
        }
        else if (ext === reports_types_1.IssueCategory.WASTE && dto.wasteDetails) {
            await client.query(`INSERT INTO report_details_waste (report_id, estimated_volume_m3, waste_type)
                 VALUES ($1, $2, $3)`, [reportId, dto.wasteDetails.estimatedVolumeM3, dto.wasteDetails.wasteType]);
        }
        else if (ext === reports_types_1.IssueCategory.BIODIVERSITY && dto.biodiversityDetails) {
            await client.query(`INSERT INTO report_details_biodiversity (report_id, species_name, observation_type, count)
                 VALUES ($1, $2, $3, $4)`, [reportId, dto.biodiversityDetails.speciesName, dto.biodiversityDetails.observationType, dto.biodiversityDetails.count || 1]);
        }
        else if ((ext === reports_types_1.IssueCategory.AIR_QUALITY || ext === reports_types_1.IssueCategory.WATER_QUALITY) && dto.environmentDetails) {
            await client.query(`INSERT INTO report_details_environment (report_id, sensor_id, measured_value, unit)
                 VALUES ($1, $2, $3, $4)`, [reportId, dto.environmentDetails.sensorId || null, dto.environmentDetails.measuredValue, dto.environmentDetails.unit]);
        }
    }
    // ── READ ────────────────────────────────────────────────────────────────
    async getReports(filters) {
        ...
        const data = result.rows.map((row) => this.mapReport(row));
        return {
            data,
            total,
            page,
    ...
    async updateReport(id, dto) {
        ...
        if (dto.issueCategory !== undefined) {
            fields.push(`issue_category = $${idx++}`);
            params.push(dto.issueCategory);
        }
        ...
        if (dto.resolvedAt !== undefined) {
            ...
            params.push(dto.resolvedAt);
        }
        if (dto.slaHours !== undefined) {
            fields.push(`sla_hours = $${idx++}`);
            params.push(dto.slaHours);
        }
        if (fields.length === 0)
    ...
    // ── COMMENTS ────────────────────────────────────────────────────────────
    async addComment(reportId, authorId, dto) {
        const sql = `
        ...
        const userResult = await this.db.query(`SELECT first_name, last_name FROM users WHERE id = $1`, [authorId]);
    ...
    // ── ASSIGNMENTS ─────────────────────────────────────────────────────────
    async assignReport(reportId, assignedBy, dto) {
        const sql = `
        ...
        if (dto.assignedTo) {
            await this.db.query(`UPDATE technician_reports SET assigned_to = $1, status = 'assigned', updated_at = NOW() WHERE id = $2`, [dto.assignedTo, reportId]);
        }
    }
    // ── MAPPING ─────────────────────────────────────────────────────────────
    mapReport(row) {
