...
const crypto_1 = __importDefault(require("crypto")); // Importé pour générer un token sécurisé pour le lien
class RegisterService {
    constructor(authRepository, logger, authMailer, hash, whatsappService) {
     ...
     */
    async registerUser(dto, credentials, isPasswordCreateRole, role) {
        try {
            ...
            if (existingEmail)
                throw new appErrors_1.ConflictError('Email already in use');
            if (normalizedPhone) {
                const existingPhone = await this.authRepository.getUserWithCredentialsByIdentifier(normalizedPhone);
                if (existingPhone)
                    throw new appErrors_1.ConflictError('Phone number already in use');
            }
            ...
            throw new appErrors_1.AppError('An unexpected error occurred during registration', 500);
        }
    }
}
