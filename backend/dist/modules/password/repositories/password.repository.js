...
class PasswordRepository {
    ...
    async getUserByEmail(email) {
        try {
            ...
            throw new appErrors_1.BadRequestError('Erreur lors de la recherche de l\'utilisateur');
        }
    }
    /**
     ...
     */
    async savePasswordResetOtp(userId, codeHash, expiresAt) {
        const client = await this.db.getClient();
        try {
            await client.query('BEGIN');
            // Invalider les anciens OTPs de même type
            await client.query(`UPDATE user_otps SET verified = true, verified_at = NOW() WHERE user_id = $1 AND otp_type = 'password_reset' AND verified = false`, [userId]);
            // Insérer le nouvel OTP
            await client.query(`INSERT INTO user_otps (id, user_id, code_hash, otp_type, expires_at) VALUES ($1, $2, $3, 'password_reset', $4)`, [crypto_1.default.randomUUID(), userId, codeHash, expiresAt]);
            await client.query('COMMIT');
        }
        catch (error) {
            await client.query('ROLLBACK');
            this.logger.error('PasswordRepository.savePasswordResetOtp error:', error);
            throw new appErrors_1.BadRequestError('Erreur lors de la sauvegarde du code OTP');
        }
        finally {
            client.release();
        }
     ...
     */
    async getValidPasswordResetOtp(userId) {
        try {
            const sql = `
            ...
            throw new appErrors_1.BadRequestError('Erreur lors de la vérification du code OTP');
        }
    }
    /**
    ...
    async resetPassword(userId, otpId, newPasswordHash) {
        ...
        try {
            ...
            await client.query(`UPDATE credentials SET password_hash = $1, updated_at = NOW() WHERE user_id = $2`, [newPasswordHash, userId]);
            // 2. Invalider l'OTP
            ...
            throw new appErrors_1.BadRequestError('Erreur lors de la réinitialisation du mot de passe');
        }
        finally {
            client.release();
        }
     ...
     */
    async getCurrentPasswordHash(userId) {
        try {
            ...
            throw new appErrors_1.BadRequestError('Erreur lors de la récupération des informations de sécurité');
        }
    }
    /**
    ...
    async updatePassword(userId, newPasswordHash) {
        try {
            await this.db.query(`UPDATE credentials SET password_hash = $1, updated_at = NOW() WHERE user_id = $2`, [newPasswordHash, userId]);
        }
        catch (error) {
            this.logger.error('PasswordRepository.updatePassword error:', error);
            throw new appErrors_1.BadRequestError('Erreur lors de la mise à jour du mot de passe');
        }
    }
    /**
     ...
     */
    async getValidSetupToken(userId, token) {
        try {
            const sql = `
                SELECT id
                FROM user_otps
            ...
            throw new appErrors_1.BadRequestError('Erreur lors de la vérification du token');
        }
    }
    /**
     ...
     */
    async setupPassword(userId, otpId, newPasswordHash) {
        const client = await this.db.getClient();
        try {
            await client.query('BEGIN');
            // 1. Mettre à jour le mot de passe
            ...
            await client.query(`UPDATE users SET status = 'active', updated_at = NOW() WHERE id = $1 AND status = 'pending'`, [userId]);
            await client.query('COMMIT');
        }
        catch (error) {
            await client.query('ROLLBACK');
            this.logger.error('PasswordRepository.setupPassword error:', error);
            throw new appErrors_1.BadRequestError('Erreur lors de la configuration du mot de passe');
        }
        finally {
            client.release();
        }
