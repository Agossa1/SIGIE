    ...
    async execute(dto) {
        ...
        if (dto.photoBase64 && dto.photoBase64.startsWith('data:image')) {
            try {
                ...
                const { url } = await this.mediaService.processAndSaveImage(fileMock, dto.createdBy);
        ...
        const reportId = await this.repository.createReport(dto);
        // 3. Invalidation du cache Redis
        await redis_service_1.redisService.del('fieldops:reports:all');
        return { id: reportId };
    }
}
