...
async function run() {
    ...
    try {
        ...
        if (orgResult.rows.length === 0) {
            throw new Error("No organization found. Please run the seed-benin.ts script first.");
        }
        const organizationId = orgResult.rows[0].id;
        console.log(`Using Organization ID: ${organizationId}`);

        // 2. Charger le fichier GeoJSON
        const geojsonPath = path.resolve(__dirname, '../src/data/ben_admin_boundaries.geojson/ben_admin2.geojson');
        ...
        for (const feature of features) {
            ...
            if (matchedMun) {
                // Mise à jour de la commune existante (met à jour le code, le département et la géométrie)
                const updateSql = `
        ...
        console.error('Error during boundary import:', e);
    } finally {
        client.release();
        await pool.end();
    }
