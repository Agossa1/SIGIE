...
export class UsersRepository {
    ...
    async findAll(): Promise<UserWithRoles[]> {
        try {
            ...
            `;
            const result = await this.db.query<UserWithRoles>(sql);
            return result.rows;
        } catch (error) {
    ...
    async findById(id: string): Promise<UserWithRoles | null> {
        try {
            const sql = `
                ...
                GROUP BY u.id, ft.id, ft.name
            `;
            const result = await this.db.query<UserWithRoles>(sql, [id]);
    ...
    async update(id: string, data: Partial<UserWithRoles>): Promise<UserWithRoles | null> {
        try {
            const sets: string[] = [];
            const values: any[] = [];
            let i = 1;
    ...
    async assignRole(userId: string, roleName: string): Promise<UserWithRoles | null> {
        ...
        try {
            ...
            if (roleResult.rows.length === 0) {
                throw new BadRequestError(`Rôle introuvable : ${roleName}`);
            }
            ...
            const updatedResult = await this.db.query<UserWithRoles>(
                `SELECT 
