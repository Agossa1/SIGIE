...
interface MayorNavbarProps {
  currentUser: User | null
  onMenuOpen: () => void
}

const MayorNavbar = ({ currentUser, onMenuOpen }: MayorNavbarProps) => {
  const displayRole = "Maire / municipalité"
    ...
    <header className="h-16 bg-white border-b border-gray-100 px-4 sm:px-6 flex items-center justify-between sticky top-0 z-30">
      <div className="flex items-center gap-3 flex-1 min-w-0">
        ...
        <div className="relative w-full max-w-sm hidden md:block">
          ...
          <input
            type="text"
            placeholder="Rechercher signalement, quartier, ouvrage..."
      ...
      <div className="flex items-center gap-2 sm:gap-3 shrink-0">
        <div className="hidden lg:flex flex-col text-right mr-2">
          <span className="text-sm font-medium text-gray-650 leading-none">Commune</span>
          <span className="text-sm font-medium text-gray-500 leading-none mt-1">Périmètre municipal</span>
        </div>
          ...
          <div className="flex items-center gap-2.5 pl-1">
            ...
            <div className="text-left hidden lg:block">
              <p className="text-sm font-medium text-black leading-tight">
                {currentUser.firstName || "Maire"} {currentUser.lastName || ""}
              </p>
