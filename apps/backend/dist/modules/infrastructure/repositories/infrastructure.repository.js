...
class InfrastructureRepository {
    ...
    async getAll(filters, page, limit) {
        ...
        let idx = 1;
        if (filters.type) {
            conditions.push(`infrastructure_type = $${idx++}`);
        ...
        if (filters.status) {
            conditions.push(`condition_status = $${idx++}`);
    ...
    async create(data) {
        const result = await this.db.query(`
            ...
            VALUES (gen_random_uuid(), $1, $2, $3, $4, $5, ST_SetSRID(ST_MakePoint($7, $6), 4326), $8, NOW(), NOW()) RETURNING *`, [data.name, data.infrastructureType, data.description, data.municipalityId, data.conditionStatus || 'good', data.latitude, data.longitude, data.userId]);
    ...
    async update(id, data) {
        ...
        if (data.conditionStatus !== undefined) {
            ...
            params.push(data.conditionStatus);
        }
...
//# sourceMappingURL=infrastructure.repository.js.map