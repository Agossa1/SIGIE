...
class LoginService {
    constructor(authRepository, logger, passwordService, tokenService) {
    ...
    async login(identifier, password) {
        try {
            ...
            if (!user) {
                ...
                throw new appErrors_1.UnauthorizedError('Identifiants invalides');
            }
            // 2. Vérification du statut (Actif ?)
            if (user.status === auth_types_1.UserStatus.PENDING) {
                this.logger.warn(`Login failed: account not verified for ${cleanIdentifier}`);
                throw new appErrors_1.ForbiddenError('Veuillez vérifier votre compte avant de vous connecter');
            }
            if (user.status !== auth_types_1.UserStatus.ACTIVE) {
                this.logger.warn(`Login failed: account status is ${user.status} for ${cleanIdentifier}`);
                throw new appErrors_1.ForbiddenError('Votre compte est suspendu ou désactivé');
            }
            // 3. Vérification du mot de passe
            if (!user.credentials) {
                throw new appErrors_1.UnauthorizedError('Identifiants invalides');
            }
            const isPasswordValid = await this.passwordService.comparePassword(password, user.credentials.passwordHash);
            if (!isPasswordValid) {
            ...
            throw new appErrors_1.AppError('Une erreur est survenue lors de la connexion', 500);
        }
    }
}
