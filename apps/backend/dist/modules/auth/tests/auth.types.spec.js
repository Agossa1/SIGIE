...
describe('AccountType enum — doit correspondre à type_account_enum', () => {
    ...
    test('contient toutes les valeurs du schéma type_account_enum', () => {
        const enumValues = Object.values(auth_types_1.AccountType);
        expectedValues.forEach(value => {
            expect(enumValues).toContain(value);
        });
    });
    test('contient exactement 11 valeurs (pas de valeur orpheline)', () => {
        expect(Object.values(auth_types_1.AccountType).length).toBe(11);
    });
    test("inclut 'citizen' pour les signalements citoyens (citizen_reports)", () => {
        expect(auth_types_1.AccountType.CITIZEN).toBe('citizen');
    });
});
describe('Role enum — doit correspondre à user_role_enum', () => {
    ...
    test('contient toutes les valeurs du schéma user_role_enum (11 rôles)', () => {
        const enumValues = Object.values(auth_types_1.Role);
    ...
    test('chaque valeur est une chaîne snake_case cohérente', () => {
        Object.values(auth_types_1.Role).forEach(role => {
...
describe('UserStatus enum — doit correspondre à user_status_enum', () => {
    ...
    test('contient exactement 4 statuts', () => {
        expect(Object.values(auth_types_1.UserStatus).length).toBe(4);
        expectedValues.forEach(value => {
            expect(Object.values(auth_types_1.UserStatus)).toContain(value);
        });
    });
});
// ============================================================================
...
describe('User interface — doit refléter la table users', () => {
    test("contient tous les champs obligatoires de la table 'users'", () => {
        const user = {
            ...
            type: auth_types_1.AccountType.USER,
    ...
    test('accepte les champs territoriaux complets (region → neighborhood)', () => {
        const user = {
            ...
            type: auth_types_1.AccountType.TECHNICIAN,
    ...
    test('accepte le type CITIZEN pour les utilisateurs de citizen_reports', () => {
        const user = {
            ...
            type: auth_types_1.AccountType.CITIZEN,
            status: auth_types_1.UserStatus.ACTIVE,
            createdAt: new Date(),
            updatedAt: new Date(),
        };
...
describe('AuthUser interface — agrégation correcte', () => {
    test('étend User + credentials + roles + sessions + otps', () => {
        const authUser = {
            ...
            type: auth_types_1.AccountType.ADMIN,
            status: auth_types_1.UserStatus.ACTIVE,
            createdAt: new Date(),
            updatedAt: new Date(),
            roles: [auth_types_1.Role.SUPER_ADMIN],
            credentials: {
                id: 'cred-1',
                userId: 'user-1',
                passwordHash: 'hash',
        ...
        expect(authUser.roles).toContain(auth_types_1.Role.SUPER_ADMIN);
...
describe('TokenPayload interface — doit refléter le JWT payload', () => {
    test('contient tous les champs territoriaux pour le contexte utilisateur', () => {
        const payload = {
            id: 'user-1',
            email: 'user@test.com',
            roles: [auth_types_1.Role.TECHNICIAN],
            organizationId: 'org-1',
            regionId: 'reg-1',
            municipalityId: 'mun-1',
            districtId: 'dist-1',
        ...
        expect(payload.roles).toContain(auth_types_1.Role.TECHNICIAN);
    });
});
    ...
    test('expose accessToken + refreshToken sans credentials', () => {
        const response = {
            user: {
                ...
                createdAt: new Date(),
                updatedAt: new Date(),
                roles: [auth_types_1.Role.VIEWER],
                sessions: [],
                otps: [],
            },
        ...
        expect(response.user.credentials).toBeUndefined();
...
describe('UpdateUserDTO interface — champs modifiables', () => {
    test('permet la mise à jour des champs de base', () => {
        ...
        expect(dto.status).toBe(auth_types_1.UserStatus.ACTIVE);
    });
});
//# sourceMappingURL=auth.types.spec.js.map