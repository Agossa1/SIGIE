...
class AuthRepository {
     ...
     */
    async createUser(dto, credentials, role) {
        ...
        try {
            ...
            const params = [
                ...
                dto.email,
                dto.phone,
                dto.type || auth_types_1.AccountType.USER,
                dto.status || auth_types_1.UserStatus.PENDING,
                dto.createdBy ?? null,
                now,
                now
            ];
            ...
            if (roleResult.rows.length > 0) {
                ...
                throw new appErrors_1.BadRequestError(`Le rôle '${role}' n'existe pas dans le système. Vérifiez la configuration des rôles.`);
            }
            ...
            throw new appErrors_1.BadRequestError('Failed to create user account');
        }
        finally {
            client.release();
        }
     ...
     */
    async getUserWithCredentialsByIdentifier(identifier) {
        try {
            const sql = `
                SELECT 
            ...
            throw new appErrors_1.BadRequestError('Failed to fetch user credentials');
        }
    }
    /**
    ...
    async createSession(userId, token, expiresAt, ipAddress, userAgent) {
        try {
            ...
            throw new appErrors_1.BadRequestError('Failed to create session');
        }
    }
    /**
    ...
    async getSessionByToken(token) {
        try {
            ...
            throw new appErrors_1.BadRequestError('Session non trouvée ou expirée');
        }
    }
    /**
    ...
    async deleteSession(token) {
        try {
            ...
            throw new appErrors_1.BadRequestError('Erreur lors de la déconnexion');
        }
    }
    /**
    ...
    async deleteUserSessions(userId) {
        try {
            ...
            throw new appErrors_1.BadRequestError('Erreur lors de la réinitialisation des sessions');
        }
    }
    /**
     * Vérifie le compte utilisateur via OTP et l'active.
     */
    async verifyUserAccount(emailStr, code) {
        const client = await this.db.getClient();
        try {
            await client.query('BEGIN');
     ...
     */
    async saveOtpCode(userId, codeHash, otpType, expiresAt) {
        try {
            const sql = `
                INSERT INTO user_otps (id, user_id, code_hash, otp_type, expires_at) 
                VALUES ($1, $2, $3, $4, $5)`;
            await this.db.query(sql, [crypto_1.default.randomUUID(), userId, codeHash, otpType, expiresAt]);
        }
        catch (error) {
            this.logger.error('Error saving OTP code:', error);
            throw new appErrors_1.BadRequestError('Failed to save verification code');
        }
    }
    async getUserByEmail(email) {
        try {
            ...
            throw new appErrors_1.BadRequestError('Failed to fetch user by email');
        }
    }
    /**
     * Met à jour le statut d'un utilisateur et log le changement dans l'historique.
     */
    async updateUserStatus(userId, newStatus, changedBy, reason) {
        const client = await this.db.getClient();
        try {
            await client.query('BEGIN');
            ...
            throw new appErrors_1.BadRequestError('Failed to update user status');
        }
        finally {
            client.release();
        }
    ...
    async getUserPreferences(userId) {
        try {
            ...
            throw new appErrors_1.BadRequestError('Failed to fetch user preferences');
        }
    }
    /**
